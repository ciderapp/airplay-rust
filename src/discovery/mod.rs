...
pub struct AirplayDevice {
    ...
    pub security: AirPlaySecurity,
    pub transient: bool,
    pub sonos_mfi: bool
}

pub fn get_airplay_devices() -> Vec<AirplayDevice> {
    // Create a daemon
    ...
    for device in all_devices {
            ...
            if airplay_devices.len() == 0 {
                airplay_devices.push(airtune_device);
            }
            else {
                let mut found = false;
                for mut airplay_device in airplay_devices.iter_mut() {
                    if airplay_device.host == airtune_device.host && airplay_device.port == airtune_device.port.to_owned() {
                        airplay_device = &mut airtune_device;
                        found = true;
                        break;
                    }
                }
